# Documentation Index

## Core
- `ARCHITECTURE.md`
- `PRINCIPLES.md`
- `SECURITY.md`
- `TESTING.md`
- `RECOVERY.md`
- `PERFORMANCE.md`
- `SOURCES.md`

## Setup
- See [Choose only the stages you want](../README.md#choose-only-the-stages-you-want) for the interactive setup builder and selective provisioning.
- `setup/PRE-WIPE.md`
- `setup/NEW-DECK.md`
- `setup/USB-OFFLINE-INSTALL.md`
- `setup/FIRST-RUN.md`

## AI
- `ai/COMMON.md`
- `ai/WORKFLOW.md`
- `ai/TASK-TEMPLATE.md`

## Modules
Each module's canonical domain documentation is `modules/<module>/README.md`.

## Android
- `decisions/ADR-007-android-waydroid.md`

## Media
- `decisions/ADR-008-media-web-apps.md`

## Host companions
- [Windows Sunshine Host Kit](host/SUNSHINE-WINDOWS.md)

## New lifecycle modules
- `modules/controller/README.md` — Steam Input capture/templates/recommendations
- `modules/library/README.md` — Steam non-Steam shortcut/artwork audit
- `modules/utilities/README.md` — Flatseal/recovery utilities

- [v0.2.22 provisioning completion](RELEASE-0.2.22.md)

- [v0.2.23 existing-install preflight fix](RELEASE-0.2.23.md)

- [v0.2.24 installer permission fix](RELEASE-0.2.24.md)

- [v0.2.25 release notes](RELEASE-0.2.25.md)

- [v0.2.26 release notes](RELEASE-0.2.26.md)
- [Provisioning and recovery](OPERATIONS.md)
- [GitHub distribution](GITHUB-RELEASES.md)

## Optional dashboard containers

- [Docker/Compose setup, SSH engines and dashboard workflow](../modules/dev/containers/README.md)

- [v0.2.27 release notes](RELEASE-0.2.27.md)

- [Docker everyday commands](DOCKER.md)
- [App selection and removal](APPS.md)
- [v0.2.39 release notes](RELEASE-0.2.39.md)
- [v0.2.43 — Clearer setup completion and retries](RELEASE-0.2.43.md)
- [v0.2.42 — Install progress and shared appearance](RELEASE-0.2.42.md)
- [v0.2.41 — Setup sharing and per-item installation](RELEASE-0.2.41.md)
- [v0.2.40 — Decky theme palette selection](RELEASE-0.2.40.md)
- [v0.2.38 release notes](RELEASE-0.2.38.md)
- [v0.2.37 release notes](RELEASE-0.2.37.md)
- [v0.2.36 release notes](RELEASE-0.2.36.md)
- [v0.2.35 release notes](RELEASE-0.2.35.md)
- [v0.2.34 release notes](RELEASE-0.2.34.md)
- [v0.2.33 release notes](RELEASE-0.2.33.md)
- [v0.2.32 release notes](RELEASE-0.2.32.md)
- [v0.2.31 release notes](RELEASE-0.2.31.md)
- [v0.2.30 release notes](RELEASE-0.2.30.md)
- [v0.2.29 release notes](RELEASE-0.2.29.md)
- [v0.2.28 release notes](RELEASE-0.2.28.md)

- [Sharing and resuming a custom setup](setup/SHARING-AND-RESUME.md)
