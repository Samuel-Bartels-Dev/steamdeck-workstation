...
Share a saved setup through **More → Export saved setup**, or import someone else's
choices and review each selection before installing. Review offers **Check downloads
& space**; **Install & finish** keeps per-item results and targeted retries across
restarts. See [sharing and resuming setup](docs/setup/SHARING-AND-RESUME.md).
...
When Decky plugin installation needs administrator permission, setup opens a KDE password dialog before starting the installation queue. CSS changes use CSS Loader’s live connection without sudo or restarting Decky, including in Nested Desktop. Missing plugin installations remain deferred there to protect the Steam session. If live CSS activation is unavailable, enable Standalone Backend in CSS Loader settings and retry; setup never falls back to restarting Steam or Decky for CSS. The password goes directly to sudo and is never saved or logged by deckctl. Authorization ends with the run; an expired ticket may require another prompt. Cancel leaves those items retryable while independent apps can continue. Initial vendor setup wizards still have their own interactive steps.
