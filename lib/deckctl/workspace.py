...
def status(json_mode=False):
    data={sid:(APPS/f'deck-workspace-{sid}.desktop').exists() for sid in SERVICES}
    if json_mode: print(json.dumps(data,indent=2))
    else:
        print('Workspace apps\n--------------')
        for sid,(name,_) in SERVICES.items(): print(f"{name:10} {'READY' if data[sid] else 'MISSING'}")
...
def verify():
    ready = all((APPS / f'deck-workspace-{sid}.desktop').is_file() for sid in SERVICES)
