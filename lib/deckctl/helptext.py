...
PAGES = {
'setup install': ('Install saved choices with persistent per-item results.', 'Runs selected items and dependencies on demand. --resume rechecks completed items before skipping them. --item ID retries one selected item and its dependencies. Exit 2 means user setup is still needed. No startup service is created.', 'setup install --resume'),
'setup preview': ('Inspect the saved installation plan without installing software.', 'Reports dependencies, installed evidence and staging allowances. --online checks supported provider metadata. Unavailable update information stays unknown; provider installers check unknown sizes.', 'setup preview --online'),
'ai-workspace install': ('Install user-space Ollama, configure OpenCode and pre-pull your selected Qwen coding models.', 'Downloads the upstream Linux archive and libraries without sudo, validates its SHA-256 and installs the raw executable in ~/.local/bin. Choose lightweight 1.5B, higher-quality 7B, or both in setup. Complete models are compared with upstream manifest metadata; unchanged models are skipped. Disk space is checked per destination filesystem before runtime/model downloads. Temporarily starts an owned local server for each model pull and stops it afterward. Preserves unrelated configuration; conflicts fail before mutation. Terminal tools are installed by the terminal module.', 'ai-workspace install'),
'ai-workspace open': ('Open a foreground, on-demand AI coding session.', 'Use --model qwen2.5-coder:1.5b or --model qwen2.5-coder:7b to choose an installed local model. Without --model, use the selected model (lightweight first when both are selected). local-ollama starts a dedicated loopback Ollama process with OLLAMA_KEEP_ALIVE=0 and stops its process group on exit or handled signals. chatgpt-pro starts no Ollama process and requires an OpenAI model ID after browser sign-in. Never borrows or stops an existing server. Closing a detached tmux client does not end its workspace; exit the session itself.', 'ai-workspace open'),
'ai-workspace status': ('Inspect local AI workspace installation without starting an engine.', 'Reports executables, config, model manifest/blob presence and dedicated endpoint occupancy. Occupancy is not proof of ownership or memory usage. Account authentication, model quality and GPU acceleration are not certified.', 'ai-workspace status --json'),
'ai-workspace guide': ('Print workspace launch and supported ChatGPT subscription sign-in instructions.', 'Read-only. OpenCode uses its OpenAI ChatGPT Plus/Pro browser sign-in flow. No browser access token is copied into configuration.', 'ai-workspace guide'),
'apps list': ('List selectable desktop apps, choices and installation status.', 'Read-only. Installed does not certify login or application behavior. Covers Zen, Zed, VLC, Plex, Spotify, VS Code, Flatseal, Discord and Slack.', 'apps list'),
'apps select': ('Choose desktop apps for future installation.', 'With no arguments, prompts for each app. Names replace the full selection; --none selects no apps. Saves choices without installing or removing apps. Without a saved choice all catalog apps are selected for compatibility.', 'apps select zen zed vlc'),
'apps install': ('Install selected desktop apps from Flathub.', 'No names installs the saved selection. Explicit names enable and install those apps. Existing user/system packages are reused. Failures are retryable; other selected apps are attempted.', 'apps install spotify'),
'apps uninstall': ('Preview removal of named user-installed desktop apps.', '--yes removes the named user apps and disables their future installation. Preserves app data and settings. Never removes system installations, other apps or unused runtimes. A failed removal stays deselected and can be retried.', 'apps uninstall spotify'),
...
'health': ('Summarize module, storage, backup, and operational readiness.', 'Read-only summary. Inspect individual readiness states; use verify for an aggregate automation exit status.', 'health --json'),
 ...
 'terminal apply': ('Install the user-space terminal stack and apply managed shell, prompt, font, and tmux configuration.', 'May download tools and fonts and update managed configuration. --config-only avoids tool downloads; --refresh rechecks upstream versions; current versions are not downloaded again.', 'terminal apply --config-only'),
 ...
 'desktop apply': ('Install project-owned Desktop icons and repair known project shortcut icon references.', 'Writes user icon files and known project .desktop entries only. Repairs Moonlight and Battle.net app icons while preserving their launch commands. SteamGridDB artwork files remain untouched.', 'desktop apply'),
 ...
 'setup customize': ('Build a staged workstation plan with optional features and desktop apps.', 'Desktop Mode opens a Qt Quick setup app with a stage sidebar, feature cards, app choices, review and persistent per-item results; interactive terminals get a text fallback. Base support is always selected, module dependencies are included automatically, and app selections enable their owning module when needed. Review and confirm before the plan is saved. Saving choices never installs or removes software. Save & install explicitly starts provisioning with Konsole handling vendor prompts. The local UI connection closes when the app exits. --defaults restores the repository default plan, --minimal selects base only, and repeated --module/--app flags set a custom plan.', 'setup customize'),
...
'storage_role':'Storage role associated with the registered game.',
 ...
 'refresh':'Check upstream terminal tool versions while preserving the existing installation.',
