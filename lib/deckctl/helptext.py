...
PAGES = {
'ai-workspace install': ('Install user-space Ollama, configure OpenCode and pre-pull Qwen 2.5 Coder 1.5B.', 'Downloads the upstream Linux archive and libraries without sudo, validates its SHA-256 and installs the raw executable in ~/.local/bin. Temporarily starts an owned local server for the model pull and stops it afterward. Preserves unrelated configuration; conflicts fail before mutation. Terminal tools are installed by the terminal module.', 'ai-workspace install'),
'ai-workspace open': ('Open a foreground, on-demand AI coding session.', 'local-ollama starts a dedicated loopback Ollama process with OLLAMA_KEEP_ALIVE=0 and stops its process group on exit or handled signals. chatgpt-pro starts no Ollama process and requires an OpenAI model ID after browser sign-in. Never borrows or stops an existing server. Closing a detached tmux client does not end its workspace; exit the session itself.', 'ai-workspace open'),
