...
if [[ $- == *i* ]]; then
  ...
  if [[ "$_deckctl_prompt_engine" == "posh" ]] && command -v oh-my-posh >/dev/null 2>&1; then
  ...
  ll() {
    if command -v eza >/dev/null 2>&1; then eza -lah --git --icons=auto --group-directories-first "$@"; else command ls -lah "$@"; fi
  }
  lt() {
    if command -v eza >/dev/null 2>&1; then eza --tree --level=2 --icons=auto --group-directories-first "$@"; else command find "${1:-.}" -maxdepth 2 -print; fi
  }
  cat() {
    if command -v bat >/dev/null 2>&1; then bat --paging=never "$@"; else command cat "$@"; fi
  }
  ff() {
